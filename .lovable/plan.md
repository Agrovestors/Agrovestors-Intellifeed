...
- Enable **Lovable Cloud** (Supabase under the hood). All secrets (`VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, `VITE_SUPABASE_SERVICE_ROLE_KEY`) live in the platform, never in the repo. `.env` stays out of git; only `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY` are exposed to the browser.
- Client split:
